...
!------------------------------------------------------------------------------
!
!  File: module_lpse_apse.f90
!> @file
!> @breif 先求解LPSE，再求解APSE.
!  DESCRIPTION:
!>
!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
!
!  module: mod_LPSE_APSE
!> @breif 先求解LPSE后求解APSE.
!  DESCRIPTION:
!>
!!
!  REVISION HISTORY:
...
module mod_LPSE_APSE
  ...
  type, extends(solver_type), public :: lpse_apse_type
    ...
    procedure :: SetDisInlet=>SetDisInlet_IR_PSE !<设置入口扰动
    procedure :: SetWholeDis !< 设置全局求解对应的扰动
    procedure :: Print !< 输出PSE的计算结果
    procedure :: Solve=>Solve_LPSE_APSE !< 推进求解PSE
...
contains
  ...
  subroutine SetWholeDis(this, Dis_LPSE, Dis_APSE)
      ...
      implicit none
      class(lpse_apse_type), intent(inout) :: This
  ...
  !> LPSE推进求解
  subroutine Solve_LPSE_APSE(this)
      ...
      call this%SetDisInlet_IR_APSE(DisNorm%GetWaveNum(), this%Iend)
  ...
  end subroutine Solve_LPSE_APSE
  ...
  subroutine Print(this, fn_surf)
      ...
      use stringifor
      implicit none
      class(lpse_apse_type), intent(in) :: this
  ...
  subroutine SetDisInlet_IR_PSE(this, wavenum, iloc_in)
      ...
      call this%Dis_LPSE%Print(this%iloc, this%Grid)
  ...
  subroutine SetDisInlet_IR_APSE(this, wavenum, iloc_in)
      ...
      class(lpse_apse_type), intent(inout) :: this
      class(dis_wavenum_type), intent(in) :: wavenum !< 初始扰动色散特征
      integer, optional, intent(in) :: iloc_in  !< 站位序号
      integer :: iloc
      type(dis_wavenum_lpse_type) :: wavenum_pse
      type(lpse_dis_normal_type) :: DisNorm
      type(BF_normal_type), save :: BFNorm
